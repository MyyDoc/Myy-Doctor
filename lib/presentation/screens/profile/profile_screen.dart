...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Future<void> _checkIfFollowing(String targetUserId) async {
    ...
    final followingRef = FirebaseDatabase.instance.ref(
      'users/$currentUid/followingList/$targetUserId',
    );
  ...
  void _listenToFollowersAndFollowing(String targetUserId) {
    // Followers count
    final followersRef = FirebaseDatabase.instance.ref(
      'users/$targetUserId/followersList',
    );
    ...
    if (targetUserId == FirebaseAuth.instance.currentUser?.uid) {
      final followingRef = FirebaseDatabase.instance.ref(
        'users/$targetUserId/followingList',
      );
  ...
  Future<void> _toggleFollow(String targetUserId) async {
    ...
    final followersRef = FirebaseDatabase.instance.ref(
      'users/$targetUserId/followersList/$currentUid',
    );
  ...
  Widget build(BuildContext context) {
    ...
    final bool showExtraTabs = widget.userId == null;
    final int tabCount = showExtraTabs ? 3 : 1;
                ...
                return GestureDetector(
                  onTap:
                      () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => ReelPostUplodingScreen(),
                        ),
                      ),
                  child: const Padding(
  ...
  Widget profileDetailsMainContainer(
    TextTheme textTheme,
      ...
      return BlocBuilder<FetchUserDetailsCubit, FetchUserDetailsState>(
        builder: (context, state) {
          ...
          if (state is FetchUserDetailsError ||
            ...
            return Center(
              child: Text(
                state is FetchUserDetailsError
                    ? state.message
                    : "User not found",
              ),
            );
      ...
      children: [
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      if (isOwnProfile)
                        Expanded(
                          child: customContainerWidget("Subscriber Chat"),
                        ),
        ...
        Row(
          ...
          children: [
            ...
            if (widget.userId != null) const SizedBox(width: 10),
            if (widget.userId != null)
              Expanded(
                child: GestureDetector(
                  onTap: () async {
        ...
        if (widget.userId != null || isDoctor)
          BlocBuilder<FetchMyStoriesCubit, FetchMyStoriesState>(
            builder: (context, state) {
              ...
              final stories =
                  state is FetchMyStoriesSuccess
                      ? state.stories
                      : <StoryModel>[];
                      ...
                      return GestureDetector(
                        onTap: () async {
                          final result = await Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => const StoryCreatorHome(),
                            ),
                          );
                          if (result == "success") {
                            context.read<FetchMyStoriesCubit>().fetchMyStories(
